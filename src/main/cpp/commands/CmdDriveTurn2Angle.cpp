...
{
  m_calcAngle = m_container.m_drivetrain.GetGyroAngle() + m_angle;
}
...
void CmdDriveTurn2Angle::Execute() 
{
  ...
  float delta_angle = m_calcAngle - m_container.m_drivetrain.GetGyroAngle();
  ...
  if( delta_angle > 0)
    m_container.m_drivetrain.RobotcentricDrive(0,0,  -turn_power);
  else
    m_container.m_drivetrain.RobotcentricDrive(0,0,   turn_power);
