...
CmdDriveStraightGyro::CmdDriveStraightGyro(float power, float gyroHeading, float gyroYaw, float distance, bool ramp, bool stop, float timeout)
{
...
void CmdDriveStraightGyro::Execute() 
{
  ...
  float delta_angle = m_yaw - m_container.m_drivetrain.GetGyroAngle();
...
//m_container.m_drivetrain.RobotcentricDrive(y_power,x_power,0.0);
  ...
  if( delta_angle > 0)
    m_container.m_drivetrain.FieldcentricDrive( y_power, x_power, -turn_power);
  else
    m_container.m_drivetrain.FieldcentricDrive( y_power, x_power,  turn_power);
